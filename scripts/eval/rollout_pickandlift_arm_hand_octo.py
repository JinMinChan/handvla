#!/usr/bin/env python3
"""Rollout Octo arm+hand policy (arm7 + hand synergy k-D) in Franka pick-and-lift scene."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Franka arm joints and hand synergy action."
        )
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="EMA factor for arm joint targets. 1.0 matches absolute-command dataset semantics.",
    )
    parser.add_argument(
...
def _default_record_path() -> Path:
    ...
    return Path("codex/logs/videos") / f"pickandlift_arm_hand_octo_rollout_{ts}.mp4"


def _default_summary_path() -> Path:
    ts = datetime.now().strftime("%y%m%d_%H%M%S")
    return Path("codex/logs/json") / f"pickandlift_arm_hand_octo_eval_{ts}.json"
...
def run_rollout(args: argparse.Namespace) -> dict:
    ...
    try:
        ...
        for ep in range(total_rollouts):
            ...
            cached_arm_target = q_arm_cmd.copy()
            ...
            def step_with_policy(phase_id: int) -> dict:
                ...
                if repeat_left <= 0:
                    ...
                    arm_pred = pred[:7]
                    hand_pred = pred[7:]
                    cached_arm_target = np.clip(arm_pred, arm_cfg.q_min, arm_cfg.q_max)
                ...
                q_arm_cmd = (1.0 - arm_alpha) * q_arm_cmd + arm_alpha * cached_arm_target
    ...
    summary = {
        ...
        "action_interface": "arm_joint7_hand_synergy_kd_e2e",
...
def main() -> None:
    ...
    print(
        "\n=== PickAndLift Arm+Hand Octo Rollout Summary ===\n"
