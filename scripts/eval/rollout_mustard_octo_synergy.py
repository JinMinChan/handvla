#!/usr/bin/env python3
"""Run closed-loop rollout for joint-synergy-kD finetuned Octo model."""
...
from rollout_mustard_octo import _build_joint_indices, _find_stats, _make_state_vector
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Rollout finetuned Octo policy in joint-synergy-kD action mode."
    )
    parser.add_argument(
        "--model-path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to full_joint PCA basis npz (contains mu and B).",
    )
    parser.add_argument(
        "--dataset-name",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.35,
        help="EMA blend for decoded joint targets (1.0 means direct decode).",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Output path (default: codex/logs/rollout_mustard_octo_synergy_<timestamp>.mp4).",
    )
...
def _default_record_path() -> Path:
    ...
    return Path("codex/logs") / f"rollout_mustard_octo_synergy_{ts}.mp4"


def _default_summary_path() -> Path:
    ts = datetime.now().strftime("%y%m%d_%H%M%S")
    return Path("codex/logs") / f"synergy_eval_summary_{ts}.json"
        ...
        for ep in range(args.episodes):
            ...
            for p_step in range(args.max_policy_steps):
                ...
                if pred.shape[-1] != k:
                    ...
                    print(
                        f"[episode {ep:03d}] expected action_dim={k}, got {pred.shape[-1]}",
                        flush=True,
                    )
                    break
                ...
                q_target = mu + B @ pred
                q_target = np.clip(q_target, hand_cfg.q_min, hand_cfg.q_max)
                ...
                z_norm = float(np.linalg.norm(pred))
            ...
            mean_z_norm = z_norm_acc / max(decision_steps, 1)
            ...
            print(
                ...
                f"mean_z_norm={mean_z_norm:.6f} mean_q_delta={mean_q_delta:.6f}",
                flush=True,
            )
...
def main() -> None:
    ...
    print(
        "\n=== Mustard Octo Synergy Rollout Summary ===\n"
