#!/usr/bin/env python3
"""Convert Franka pick-and-lift raw episodes to arm-TCP6 + hand-synergy-k actions."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Encode Franka pick-and-lift raw episodes into arm TCP pose "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "dataset/franka_pickandlift_arm_tcp_hand_synergy_k{K}_{source_suffix}"
        ),
    )
    parser.add_argument(
        "--instruction",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Append phase one-hot and progress features to the raw state vector.",
    )
    parser.add_argument(
        "--arm-action-type",
...
def _compute_phase_features(phases: np.ndarray) -> np.ndarray:
    phase_ids = np.asarray(phases, dtype=np.int32).reshape(-1)
    steps = int(phase_ids.shape[0])
    onehot = np.zeros((steps, 6), dtype=np.float32)
    onehot[np.arange(steps), np.clip(phase_ids, 0, 5)] = 1.0

    global_progress = np.zeros((steps, 1), dtype=np.float32)
    if steps > 1:
        global_progress[:, 0] = np.linspace(0.0, 1.0, steps, dtype=np.float32)

    phase_progress = np.zeros((steps, 1), dtype=np.float32)
    start = 0
    while start < steps:
        phase_id = phase_ids[start]
        end = start
        while end < steps and phase_ids[end] == phase_id:
            end += 1
        seg_len = end - start
        if seg_len > 1:
            phase_progress[start:end, 0] = np.linspace(0.0, 1.0, seg_len, dtype=np.float32)
        start = end

    return np.concatenate([onehot, phase_progress, global_progress], axis=-1).astype(np.float32)
...
def main() -> None:
    ...
    if args.out_dir:
        ...
        out_root = Path(
            f"dataset/franka_pickandlift_arm_tcp_hand_synergy_k{k}_{suffix}"
        ).resolve()
    ...
    for ep_idx, src_path in enumerate(files):
        ...
        if args.append_phase_features and ("state" not in data or "phase" not in data):
            raise KeyError(f"state/phase missing for phase feature append in {src_path}")
        ...
        if args.arm_action_type == "absolute_world":
            ...
            action_semantics = "tcp10_absolute_world_xyz_rpy_hand_synergy_kd"
            ...
            action_semantics = "tcp10_delta_local_xyz_rpy_hand_synergy_kd"
            arm_pose_representation = "palm_local_xyz_rpy_delta"
            arm_pose_err = arm_delta_local_tcp6.astype(np.float64)

        arm_hand10 = np.concatenate([arm_action_tcp6, z], axis=-1).astype(np.float32)
        ...
        if state_dim is None and "state" in data:
            ...
            if st.ndim == 2:
                extra_dim = 8 if args.append_phase_features else 0
                state_dim = int(st.shape[1] + extra_dim)
        ...
        if args.append_phase_features:
            state = np.asarray(data["state"], dtype=np.float32)[:steps]
            phase_features = _compute_phase_features(np.asarray(data["phase"], dtype=np.int32)[:steps])
            payload["state"] = np.concatenate([state, phase_features], axis=-1).astype(np.float32)
            payload["state_phase_features"] = phase_features.astype(np.float32)
            payload["state_phase_feature_dim"] = np.asarray(phase_features.shape[1], dtype=np.int32)
        ...
        payload["action"] = arm_hand10.astype(np.float32)
        ...
        payload["append_phase_features"] = np.asarray(bool(args.append_phase_features))
